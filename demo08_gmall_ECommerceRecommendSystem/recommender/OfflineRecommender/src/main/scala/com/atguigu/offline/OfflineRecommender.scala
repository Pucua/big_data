...
object OfflineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val usersProducts = userRDD.cartesian(productRDD)
    // model已训练好，把id传进去就可以得到预测评分列表RDD[Rating] (userId,productId,rating)
    val preRating = model.predict(usersProducts)
...
/*
开启MongoDB
show tables；出现ProductRecs,UserRecs
db.UserRecs.find().pretty()|count()
db.ProductRecs.find().pretty()|count()
 */
