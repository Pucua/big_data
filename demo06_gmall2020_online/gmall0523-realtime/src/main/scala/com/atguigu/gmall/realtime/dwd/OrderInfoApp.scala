...
import com.atguigu.gmall.realtime.util.{MyESUtil, MyKafkaSink, MyKafkaUtil, OffsetManagerUtil, PhoenixUtil}
...
/*    val orderInfoWithFirstFlagStream: DStream[Unit] = orderInfoDStream.map {
      orderInfo => {
        // 获取用户id
        val userId: Long = orderInfo.user_id
        // 根据id到Phoenix中查询是否下单过
    ...
    orderInfoWithFirstFlagStream.print(1000)*/

    // 方案2：分区为单位，将整个分区的数据拼接一条sql进行一次查询
    val orderInfoWithFirstFlagStream: DStream[OrderInfo] = orderInfoDStream.mapPartitions {
      orderInfoItr => {
        // 当前一个分区中所有订单的集合
        ...
        // 根据用户集合到Phoenix中查询，看那些用户下过单  坑1 字符串的拼接
        var sql: String = s"select user_id,if_consumed from user_status0523 where user_id='${userIdList.mkString("','")}'"

        // 执行SQL从Phoenix获取数据
        val userStatusList: List[JSONObject] = PhoenixUtil.queryList(sql)
        // 获取消费过的用户id   坑2 大小写 Phoenix默认转大写
     ...
     */
    // 对待处理的数据进行结构转换
    val mapDStream: DStream[(Long, OrderInfo)] = orderInfoWithFirstFlagStream.map(orderInfo=>(orderInfo.user_id,orderInfo))
    // 根据用户id对数据进行分组
    ...
    // DO =================== 5.和省份维度进行关联  ====================
    // 方案1：分区为单位对订单数据进行处理，和Phoenix中的订单表进行关联
/*    val orderInfoWithProvinceDStream: DStream[OrderInfo] = orderInfoRealDStream.mapPartitions {
    ...
    // 方案2：以采集周期为单位对数据进行处理 --->通过SQL将所有的省份查询出来
    val orderInfoWithProvinceDStream: DStream[OrderInfo] = orderInfoRealDStream.transform {
      rdd => {
        // 从Phoenix中查询所有的省份数据
        var sql: String = s"select id,name,area_code,iso_code from gmall0523_province_info"
        val provinceInfoList: List[JSONObject] = PhoenixUtil.queryList(sql)
        val provinceInfoMap: Map[String, ProvinceInfo] = provinceInfoList.map {
          provinceJsonObj => {
            // 将json对象转换为省份样例类对象
    ...
    // DO =================== 3.维护首单用户状态,保存订单到ES中  ====================
    // 如果用户为第一次消费，进行首单标记后将用户消费状态保存在HBase中，下次消费就不再是首单了
    import org.apache.phoenix.spark._
    orderInfoWithFirstFlagStream.foreachRDD{
      rdd=>{
        // 优化
        rdd.cache()
        ...
        // 注意：saveToPhoenix要求RDD存放数据的属性个数和Phoenix表中字段数必须一致
        val userStatusRDD: RDD[UserStatus] = rdd.map {
