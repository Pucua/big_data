package com.atguigu.financial_lease_realtime.app.dwd;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.atguigu.financial_lease_realtime.bean.*;
...
public class DwdBaseApp {
    public static void main(String[] args) throws Exception {
        ...
        // do 3.筛选表示业务过程的表格
        //     5张表:credit,credit_facility,credit_facility_status,reply,contract
        SingleOutputStreamOperator<JSONObject> flatMapStream = odsStream
...
//        keyedStream.print("keyed>>>");
        ...
        SingleOutputStreamOperator<DwdAuditApprovalBean> processStream = keyedStream.process(new KeyedProcessFunction<String, JSONObject, DwdAuditApprovalBean>() {
            ...
            // 制作合同
            private ValueState<DwdLeaseContractProducedBean> contractProducedBeanStatus;
            ...
            public void open(Configuration parameters) throws Exception {
                ...
                // 设置状态的存活时间,Descriptor.enableTimeToLive 30s 避免长时间占位
//                creditAddBeanStatus = getRuntimeContext().getState(new ValueStateDescriptor<DwdCreditAddBean>("audit_add_status", DwdCreditAddBean.class));
                ...
                contractProducedBeanStatus = getRuntimeContext().getState(contractProducedStatusDescriptor);
            }
                        ...
                        switch (status) {
                            case "16": {
                                ...
                                if (replyBean == null) {
                                    ...
                                    // 如果新增授信数据先到，已经存到状态中，这里需要进行处理将对应的数据写出到侧输出流
                                    ...
                                    // 如果完成授信占用的数据先到，已经存到状态中，这里需要进行处理将对应的数据写出到侧输出流
                                    ...
                                    // 如果完成合同制作的数据先到，已经存到状态中，这里需要进行处理将对应的数据写出到侧输出流
                                    DwdLeaseContractProducedBean contractProducedBean = contractProducedBeanStatus.value();
                                    if (contractProducedBean != null) {
                                        ...
                                        contractProducedBeanStatus.clear();
                                    }
                                }
                    ...
                    if ("insert".equals(type)) {
                        ...
                        if ("2".equals(status)) {
                            ...
                            if (auditApprovalBean != null) {
                                ...
                                if (auditApprovalBean.getReplyAmount() == null) {
                                    // 批复的数据未到
                                    contractProducedBeanStatus.update(contractProducedBean);
                                } else {
                                    // 批复的数据也到了，审批的数据完整
                                    contractProducedBean.setApplyAmount(auditApprovalBean.getApplyAmount());
                                    contractProducedBean.setReplyAmount(auditApprovalBean.getReplyAmount());
...
//        cancelStream.print("cancel>>>");
        ...
        // 6.1 审批通过数据
// Caused by: java.lang.ClassCastException: com.atguigu.financial_lease_realtime.bean.DwdAuditRejectBean cannot be cast to com.atguigu.financial_lease_realtime.bean.DwdAuditApprovalBean. Failed to push OutputTag with id 'reject_stream' to operator. This can occur when multiple OutputTags with different types but identical names are being used.
// 使用了lambda表达式
//   map(JSON::toJSONString)   // processStream.map(bean -> JSON.toJSONString(bean));  简写
        processStream
                .map(new MapFunction<DwdAuditApprovalBean,String>() {
                        @Override
                        public String map(DwdAuditApprovalBean value) throws Exception {
                            return JSON.toJSONString(value);
                        }
                    })   // processStream.map(bean -> JSON.toJSONString(bean));  简写
                .sinkTo(KafkaUtil.getKafkaProducer(approveTopic,approveTopic+"_sink"))
                .name("approve_stream_sink");

        // 6.2 审批拒绝数据
        rejectStream
                .map(new MapFunction<DwdAuditRejectBean, String>() {
                        @Override
        ...
        // 6.3 审批取消数据
        cancelStream
                .map(new MapFunction<DwdAuditCancelBean, String>() {
                        @Override
        ...
        // 7.1 新增授信数据
        creditAddStream
                .map(new MapFunction<DwdCreditAddBean, String>() {
                    @Override
        ...
        // 7.2 完成授信占用
        creditOccupyStream
                .map(new MapFunction<DwdCreditOccupyBean, String>() {
                    @Override
        ...
        contractProducedStream
                .map(new MapFunction<DwdLeaseContractProducedBean, String>() {
                    @Override
        ...
        // do 10.数据分流
        OutputTag<DwdLeaseExecutionBean> executionTag= new OutputTag<DwdLeaseExecutionBean>("execution_produced_stream_"){
        };
        ...
        SingleOutputStreamOperator<DwdLeaseSignedBean> signedBeanStream = connectStream.process(new KeyedCoProcessFunction<String, DwdLeaseContractProducedBean, JSONObject, DwdLeaseSignedBean>() {
            ...
            // 签约数据
            private ValueState<DwdLeaseSignedBean> signedState;
            // 起租数据
            ...
            public void open(Configuration parameters) throws Exception {
                ...
                signedState = getRuntimeContext().getState(new ValueStateDescriptor<DwdLeaseSignedBean>("sign_state", DwdLeaseSignedBean.class));
            ...
            public void processElement1(DwdLeaseContractProducedBean bean, Context ctx, Collector<DwdLeaseSignedBean> out) throws Exception {
                // 当前为合同制作数据，判断合同表数据是否先到进行处理
                DwdLeaseSignedBean signedBean = signedState.value();
                DwdLeaseExecutionBean executionBean = executionState.value();

                if (signedBean == null || executionBean == null) {
                    // 任意一个为空表示数据不完整，后续需要使用到合同制作的数据
            ...
            public void processElement2(JSONObject jsonObject, Context ctx, Collector<DwdLeaseSignedBean> out) throws Exception {
                ...
                if ("update".equals(type)) {
                    ...
                    if ("2".equals(status)) {
                        ...
                        if (contractProducedBean == null) {
                            signedState.update(leaseSignedBean);
                        } else {
        ...
        // do 11.提取起租流，将签约和起租数据写出到对应的kafka主题
        // 11.1 写出签约数据
        String signedTopic = "financial_dwd_lease_sign";

        signedBeanStream
                .map(new MapFunction<DwdLeaseSignedBean,String>() {
                    @Override
        ...
        executionStream
                .map(new MapFunction<DwdLeaseExecutionBean, String>() {
                    @Override
...
/*
测试：
kafka-console-consumer.sh --bootstrap-server node1:9092 --topic financial_dwd_audit_reject
有数据输出
 */